...
# 1. CoPilot mode: Sets up Wine for full effect compilation in CoPilot environments
# 2. Wine mode: Sets up Wine for full effect compilation in development environments
...
setup_copilot_mode() {
    ...
    if ! command -v wine &> /dev/null; then
        echo "Installing Wine for effect compilation..."
        sudo apt update
    ...
    echo ""
    echo "Wine has been set up for full effect compilation capability."
    ...
    echo ""
    echo "You can now build MonoGame projects with full effect compilation:"
    echo "  dotnet build"
}
...
auto_detect_mode() {
    ...
    if [[ -f /.dockerenv ]] || [[ "$USER" == "runner" ]] || [[ -n "$GITHUB_ACTIONS" ]] || [[ -n "$COPILOT" ]]; then
        echo "Detected container/CI environment - using CoPilot mode (full Wine setup)"
        setup_copilot_mode
    else
